...
class ToggleElemnt {
    ...
    constructor(class_css) {
     ...
     addClass(element) {
        ...
        window.document.addEventListener('click', function _listener(){
                this.closeClick(event,element)}.bind(this,element), false);
    }
    ...
    async toggle(element){
     ...
     addEvent(){
        this.elements.forEach(function(element) {
            element.addEventListener('click', function(){this.toggle(element)}.bind(this),false);
        }.bind(this))
    }
...
class Auxiliary {
    ...
    hiddenElements(){
        this.targets_class.forEach(function(item){
    ...
    async toggleElement(){
        this.elements_target.forEach(function(item,index){
    ...
    addEvent(){
            ...
            this.elements.forEach(function(element) {
                element.addEventListener(this.tp_event,this.toggleElement.bind(this),false);
            }.bind(this))
    }
...
class Menu {
  ...
  elementsLinks = []
    constructor() {
        var fields = [];
        this.elements = document.querySelectorAll('.mobile-header .navigation .item .trigger');
        this.elementsLinks = document.querySelectorAll('.mobile-header .navigation .item a.link');
        this.menuPreferences = document.querySelectorAll('.mobile-header .user .actions .trigger');
    }

    async clickUserPreferences(){
        var menu = document.querySelectorAll('.mobile-header .overflow .navigation')[0];
        var usrpref = document.querySelectorAll('.mobile-header .overflow .user-menu')[0];
        var icon = document.querySelectorAll('.mobile-header .user .actions .trigger i')[0];
    ...
    async clickMenuItem(item){
        ...
        if(icon.classList.contains("fa-minus") && !subnav.classList.contains('d-none')) {
        ...
        item.currentTarget.classList.contains('is-active') ? item.currentTarget.classList.remove('is-active') : item.currentTarget.classList.add('is-active')
        subnav.classList.contains('d-none') ? subnav.classList.remove('d-none') : subnav.classList.add('d-none');
    }

    static removeClick(){
        document.querySelectorAll('.navigation .item .is-active').forEach(function(item){
            item.classList.remove('is-active')
        });
    }
    ...
    addEvent(){
            this.elements.forEach(function(itemButton) {
                itemButton.addEventListener('click',this.clickMenuItem.bind(itemButton),false);
            }.bind(this))
...
class InputSearch{
    ...
    async initializeEvent(){
        ...
        var toggleMobileSearch = new Auxiliary('click','.mobile-subheader .accessibility .br-button.is-primary',['.mobile-header .br-search'],'d-none')
        this.hiddenButton();
  ...
  window.onload = function(){
      ...
      var toggleMenu = new Auxiliary('focus','.desktop-header #search',['.logo', '.col-acessibilidade'],'d-none');
    toggleMenu.addEvent()
      var toggleMenu = new Auxiliary('blur','.desktop-header #search',['.logo', '.col-acessibilidade'],'d-none');
      toggleMenu.addEvent()
      ...
      var toggleMobileMenu = new Auxiliary('click','.mobile-header .canvas',['.mobile-header .canvas','.mobile-header .sidemenu'],'d-none')
      toggleMobileMenu.addEvent()
