...
Em algumas situações, a família Font Awesome pode não suprir todas as necessidades de um interface. Nestas situações, pode ser criados ícones para representar as ações muito específica de uma aplicação. Essa criação pode ser feita tanto por técnicas de transformação (utilizando os próprios ícones da família) tal como o [Layering](https://fontawesome.com/v5.9.0/how-to-use/on-the-web/styling/layering); como na criação do zero de algum novo ícone. Independente da forma de criação, siga todas as características visuais e de construção da Font Awesome Solid.
