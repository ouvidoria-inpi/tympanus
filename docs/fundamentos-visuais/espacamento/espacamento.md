...
Existem 2 escalas de espaçamento no GOVBR-DS que devem ser respeitadas. Utilizando-se dessas escalas é possível criar layouts flexíveis e, ao mesmo tempo, manter a consistência visual entre diferentes serviços e produtos dentro do GOVBR-DS.
 ...
 *Propriedades básicas do Box-model*
 ...
 No GOVBR-DS, o comportamento das dimensões de um objeto é determinada pela regra: `box-sizing: border-box`, ou seja, as propriedades de largura (width) e de altura (height) incluem o tamanho do padding e do border, mas não incluem a margin.

![Box-Model no GOVBR-DS](imagens/boxmodel-govbr-ds.png)
*Comportamento das dimensões do Box-Model utilizado no GOVBR-DS*

 Observando somente as propriedades de espaçamento, podemos então categorizá-los dentro do GOVBR-DS em **Interno** e **Externo**.

![Tipos de Espaçamentos](imagens/interno-externo.png)
*Tipos de Espaçamento no GOVBR-DS: Interno e Externo*
...
**Importante**: por padrão, os espaçamentos de qualquer elemento do GOVBR-DS é de 0px, ou seja, não utiliza nenhum tipo de escala.
