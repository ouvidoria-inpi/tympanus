[Artefato Fonte de Design]: # (footer_0.0.0-dev01.xd)

# Footer (Rodapé)
Geralmente localizado na parte inferior das páginas de web ou sistema, o _footer_ (Rodapé) pode ser organizado de formas distintas. Normalmente essa organização é definida através da combinação de elementos que reforçam a identidade visual com o conteúdo a ser informado, respeitando os objetivos do negócio e as necessidades do usuário.
...
Foi projetado para este _Design System Gov.Br_ o _footer_ (Rodapé) em uma versão padrão. Seu layout pode variar em seu conteúdo, cor e forma a depender da necessidade do projeto ou ao padrão da grid ao qual o _footer_ (Rodapé) precisará se adequar.
...
A seguir, apresentamos os elementos e componentes mais utilizados no componete _footer_ (Rodapé):
...
Esta área é destinada à utilização a logo do sistema ou portal, respeitando as alturas e larguras mínimas e maximas definidos ná área de token. 
...
**IMPORTANTE:**  O componete List, em todos os ítens que o compõe, possui a altura definida como comportamento de densidade. 
...
Para o _footer_(Rodapé) o **"comportamento lista expandir"** transforma as categorias das listas em listas do layout 12 colunas em listas expansíveis com ação collapse. Cada categoria da lista, quando acionados, apresentará em seu inteior os ítens da lista que o compõe.
...
É formada pela junção das informações referentes às resdes sociais e as marcas utilizadas como assinaturas.
...
##### Logo Assinatura
Área destinada à apresentação de logos ou siglas que integrem uma empresa ou marcas parceiras que avalizam ou endossam um evento ou projeto específico.


#### 4. Área Infomacão Legal
...
Assim como a barra de Redes Sociais e Assinaturas, a Barra de Informações Legais também será ocultadas em layouts de tela com 4 colunas, tanto na versão simplificada quanto na versão padrão. 
...
**IMPORTANTE:** É necessário atentar que no "tema 2" será necessário o uso do componente separador no topo do _footer_ (Rodapé), fazendo uma delimitação do footer com a área de conteúdo. Caso haja a necessidade da utilização da barra de informações legais no tema 2, este também deverá apresentar o componente separador no topo. Como visto abaixo:
#### Layout Flúido
...
Esta organização e composião final deverá sempre respeitar o alinhamento na sua respectiva grid.
