[Artefato Fonte de Design]: # (loading_0.0.0-dev01.xd)

# Loading
Indicadores de progresso que expressam um tempo de espera não especificado ou exibem a duração de um processo.
...
Utilizado com o objetivo proporcionar feedback ao usuário durante algum processamento do sistema. Muito utilizado em envio de formulários, upload de arquivos, carregamento de dados entre outras requisições síncronas e assíncronas ao servidor.
...
**8 - Botão Cancelar (Opcional)**
    Elemento que compreende o `Componente Botão` do tipo terciário. Seu uso é opcional. Só poderá ser utilizado quando houver a possibilidade do cancelamento do carregamento pelo sistema. Este elemento só será utilizado juntamente com o `Loading Determinado`.
