...
class BRDatepicker {
  ...
  _configDatepicker ( configData ) {
    let pickerConfig = {};
    for ( let key of this._datepickerProperties ) {
  ...
  _setBehavior () {
    for ( let datepickerButton of this.component.querySelectorAll( "button.icon" ) ) {
  ...
  _validDate ( instance ) {
    let stringDate = instance.el.value;
    let range = undefined;
    let msg_error = [];
    try {
  ...
  _showError ( instance, msg_error ) {
    ...
    for ( let message of this.component.parentNode.querySelectorAll( "div.feedback.is-invalid span" ) ) {
      message.innerText = msg_error[ 0 ];
    }
  }

  _showSucess ( instance ) {
